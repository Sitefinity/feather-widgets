    ...
    public class FilterEventsByCalendar : TestArrangementBase
    {
        /// <summary>
            ...
            public void OnBeforeTestsStarts()
            {
                ...
                ServerOperations.Events().CreateCalendar(this.calendar3Guid, Calendar3Title);
                ServerOperations.Events().CreateEvent(Event3Title, string.Empty, IsAllDay, this.eventStart, this.eventEnd, this.calendar3Guid);
                var event3Item = EventsManager.GetManager()
                    .GetEvents()
                    .Where<Event>(ni => ni.Status == Telerik.Sitefinity.GenericContent.Model.ContentLifecycleStatus.Live && ni.Title == Event3Title)
                    .FirstOrDefault();
                ServerArrangementContext.GetCurrent().Values.Add("event3Id", event3Item.Id.ToString());
            }
            ...
            private readonly Guid calendar3Guid = Guid.NewGuid();
