...
{
    /// <summary>
    /// ChangeDropdownListTemplate test class.
    /// </summary>
    [TestClass]
    public class ChangeDropdownListTemplate_ : FeatherTestCase
    {
        /// <summary>
        /// UI test ChangeDropdownListTemplate
        /// </summary>
        [TestMethod,
        Owner(FeatherTeams.SitefinityTeam6),
        TestCategory(FeatherTestCategories.Bootstrap),
        TestCategory(FeatherTestCategories.Forms)]
        public void ChangeDropdownListTemplate()
        {
            BAT.Macros().NavigateTo().Modules().Forms(this.Culture);
            BAT.Wrappers().Backend().Forms().FormsDashboard().OpenFormFromTheGrid(FormName);
            BATFeather.Wrappers().Backend().Pages().PageZoneEditorWrapper().EditWidget(DropdownListFieldController);
            ...
            BATFeather.Wrappers().Backend().Forms().FormsContentScreenWrapper().WaitForFieldContent(TemplateContentFile, DropdownListFieldController);
            BAT.Wrappers().Backend().Pages().PageZoneEditorWrapper().CheckWidgetContent(DropdownListFieldController, TemplateContentFile);         
            ...
            BATFeather.Wrappers().Backend().WidgetTemplates().WidgetTemplatesCreateScreenFrameWrapper().SelectTemplate("DropdownListField (MVC)");
            ...
            BATFeather.Wrappers().Backend().ContentBlocks().ContentBlocksWrapper().SaveChanges();
            BATFeather.Wrappers().Backend().Forms().FormsContentScreenWrapper().WaitForFieldContent(TemplateContent, DropdownListFieldController);
            BAT.Wrappers().Backend().Pages().PageZoneEditorWrapper().CheckWidgetContent(DropdownListFieldController, TemplateContent);     
            BAT.Wrappers().Backend().Forms().FormsContentScreen().PublishForm();
            BAT.Macros().NavigateTo().CustomPage("~/" + PageName.ToLower(), true, this.Culture);
        ...
        private const string DropdownListFieldController = "DropdownListFieldController";
        private const string PageName = "FormPage";
        private const string TemplateName = "Write.DefaultDropdownListNew";
        private const string TemplateNameNew = "DefaultDropdownListNew";
        private const string TemplateNameFile = "DefaultDropdownListFile";
