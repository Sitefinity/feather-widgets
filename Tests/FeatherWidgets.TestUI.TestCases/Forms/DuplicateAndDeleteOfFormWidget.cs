...
{
    /// <summary>
    /// DuplicateAndDeleteOfFormWidget test class.
    /// </summary>
    [TestClass]
    public class DuplicateAndDeleteOfFormWidget_ : FeatherTestCase
    {
        /// <summary>
        /// UI test DuplicateAndDeleteOfFormWidget
        /// </summary>
        [TestMethod,
        Owner(FeatherTeams.SitefinityTeam6),
        TestCategory(FeatherTestCategories.Bootstrap),
        TestCategory(FeatherTestCategories.Forms)]
        public void DuplicateAndDeleteOfFormWidget()
        {
            ...
            BATFeather.Wrappers().Backend().Pages().PageZoneEditorWrapper().EditWidget(WidgetName);
            BATFeather.Wrappers().Backend().Widgets().WidgetDesignerWrapper().ApplyCssClasses(CssClassesToApplyNew);
            BATFeather.Wrappers().Backend().Widgets().WidgetDesignerWrapper().SaveChanges();
            ...
            BAT.Macros().NavigateTo().CustomPage("~/" + PageName.ToLower(), true, this.Culture);
            Assert.IsTrue(ActiveBrowser.ContainsText(CssClassesToApply), "Css class was not found on the page");
            Assert.IsFalse(ActiveBrowser.ContainsText(CssClassesToApplyNew), "Css class was not found on the page");
        }
