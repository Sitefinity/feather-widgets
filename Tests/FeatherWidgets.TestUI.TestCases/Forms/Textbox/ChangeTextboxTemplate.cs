...
{
    /// <summary>
    /// ChangeTextboxTemplate test class.
    /// </summary>
    [TestClass]
    public class ChangeTextboxTemplate_ : FeatherTestCase
    {
        /// <summary>
        /// UI test ChangeTextboxTemplate
        /// </summary>
        [TestMethod,
        Owner(FeatherTeams.SitefinityTeam6),
        TestCategory(FeatherTestCategories.Bootstrap),
        TestCategory(FeatherTestCategories.Forms)]
        public void ChangeTextboxTemplate()
        {
            BAT.Macros().NavigateTo().Modules().Forms(this.Culture);
            BAT.Wrappers().Backend().Forms().FormsDashboard().OpenFormFromTheGrid(FormName);
            BATFeather.Wrappers().Backend().Pages().PageZoneEditorWrapper().EditWidget(TextFieldController);
            ...
            BATFeather.Wrappers().Backend().Forms().FormsContentScreenWrapper().WaitForFieldContent(TemplateContentFile, TextFieldController);
            BAT.Wrappers().Backend().Pages().PageZoneEditorWrapper().CheckWidgetContent(TextFieldController, TemplateContentFile);         
            ...
            BATFeather.Wrappers().Backend().WidgetTemplates().WidgetTemplatesCreateScreenFrameWrapper().SelectTemplate("TextField (MVC)");
            ...
            BATFeather.Wrappers().Backend().ContentBlocks().ContentBlocksWrapper().SaveChanges();
            BATFeather.Wrappers().Backend().Forms().FormsContentScreenWrapper().WaitForFieldContent(TemplateContent, TextFieldController);
            BAT.Wrappers().Backend().Pages().PageZoneEditorWrapper().CheckWidgetContent(TextFieldController, TemplateContent);     
            BAT.Wrappers().Backend().Forms().FormsContentScreen().PublishForm();
            BAT.Macros().NavigateTo().CustomPage("~/" + PageName.ToLower(), true, this.Culture);
        ...
        private const string TextFieldController = "TextFieldController";
        private const string PageName = "FormPage";
        private const string TemplateName = "Write.DefaultTextboxNew";
        private const string TemplateNameNew = "DefaultTextboxNew";
        private const string TemplateNameFile = "DefaultTextboxFile";
