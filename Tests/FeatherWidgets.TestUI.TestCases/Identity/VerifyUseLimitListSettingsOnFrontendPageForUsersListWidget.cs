...
{
    /// <summary>
    /// VerifyUseLimitListSettingsOnFrontendPageForUsersListWidget_ test class.
    /// </summary>
    [TestClass]
    public class VerifyUseLimitListSettingsOnFrontendPageForUsersListWidget_ : FeatherTestCase
    {
        /// <summary>
        /// UI test VerifyUseLimitListSettingsOnFrontendPageForUsersListWidget
        /// </summary>
        [TestMethod,
        Owner(FeatherTeams.SitefinityTeam7),
        TestCategory(FeatherTestCategories.PagesAndContent), 
        TestCategory(FeatherTestCategories.UsersList)]
        public void VerifyUseLimitListSettingsOnFrontendPageForUsersListWidget()
        {
            ...
            BATFeather.Wrappers().Frontend().Identity().UsersListWrapper().VerifyUsersListOnHybridPage(new string[] { UserFirstLastName1, UserFirstLastName2, UserFirstLastName3 });
            Assert.IsFalse(BATFeather.Wrappers().Frontend().CommonWrapper().AreTitlesPresentOnThePageFrontend(new string[] { UserFirstLastName4, UserFirstLastName5 }));
            BAT.Macros().NavigateTo().Pages(this.Culture);
        }
